...
namespace OwnPass {
    ...
    public:
        ...
        explicit Group(std::string& name, std::__1::list<Password>& passwords) : name{name }, passwords{passwords } {}
        explicit Group(std::string&& name) : name() { this->name = std::move(name); }
        explicit Group(std::string&& name, std::__1::list<Password>&& passwords) : name{}, passwords{} {
