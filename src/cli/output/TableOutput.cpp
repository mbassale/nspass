...
namespace OwnPass::CLI::Output {
	...
	TableOutput::TableOutput(const TableOutputOptions& options, std::vector<std::string>  headers)
			:options{ options }, headers{std::move( headers )}
	{

	}

	void TableOutput::print_headers()
	{
	...
	void TableOutput::print_row(const std::vector<std::string>& row)
	{
		auto max_column_width = get_max_column_width();
		for(auto& cell : row) {
			options.out << std::left << std::setw(max_column_width) << cell << std::setfill(' ');
	...
	size_t TableOutput::get_max_column_width()
	{
